...
const makeServerRequest = (data: object, endpoint: string, method: string = 'post') => {
  ...
  return fetch(`https://tla-backend-staging.herokuapp.com/${endpoint}`, options);
};
 ...
 * }
 */
export const getUserData = async (uid: string, includePrograms = false): Promise<{ ok: boolean, data: object, error: string }> => {
  const userDataEndpoint = `https://tla-backend-staging.herokuapp.com/user/get?uid=${uid}${includePrograms ? '&programs=true' : ''
    }`;
  ...
  try {
    ...
    if (err instanceof Error)
      ...
      return { ok: false, data: {}, error: 'Error encountered' }
    // await createUser(uid);
    // return getUserData(uid, includePrograms);
  }
};
...
export const createSketch = ({ uid, wid, program }: { uid: string; wid: string; program: Program; }) => {
...
export const deleteSketch = (data: { uid: string; name: string; }) => {
  const { uid, name } = data;
 ...
 */
export const createClass = (data: { uid: string, name: string, thumbnail: string }) => makeServerRequest(data, 'class/create');
 ...
 */
export const getClass = async (data: { uid: string, cid: string }, withPrograms: boolean, withUserData: boolean) => {
